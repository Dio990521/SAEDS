...
class BufferHandler(object):
    ...
    def appendtext(self, text):
        ...
        self.fileobject.write(text.strip())
...
def cut_from_point(input, sequence_length, ind, option, mode=0):
    ...
    input_forward=np.zeros([batch_size, num_steps])+2 # 2 = EOS
    input_backward=np.zeros([batch_size, num_steps])+2
    sequence_length_forward=np.zeros([batch_size])
    sequence_length_backward=np.zeros([batch_size])
    for i in range(batch_size):
        input_forward[i][0]=0 # 0 = BOS
        input_backward[i][0]=0
...
def generate_candidate_input(input, sequence_length, ind, prob, search_size, option, mode=0):
    input_new=np.array([input]*search_size)
    sequence_length_new=np.array([sequence_length]*search_size)
    if mode!=2:
        ind_array = np.argsort(prob)
        for i in range(0,4):
            useless_index = np.argwhere(ind_array==i)
            ind_array = np.delete(ind_array, useless_index)
        ind_token = ind_array[-search_size:]

    if mode==2:
        for i in range(sequence_length-ind-2):
            input_new[: , ind+i+1]=input_new[: , ind+i+2]
        for i in range(sequence_length-1, option.num_steps-1):
            input_new[: , i]=input_new[: , i]*0 + 2 # 2 = EOS
