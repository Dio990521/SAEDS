...
class GeneratorHubInterface(nn.Module):
    ...
    def get_clm(self, src_sent, trg_sent, criterion):
        if isinstance(src_sent, str):
            src_sent = [src_sent]
            assert isinstance(trg_sent, str)
            trg_sent = [trg_sent]

        src_tokens = [self.encode(sentence) for sentence in src_sent]
        trg_tokens = [self.encode(sentence) for sentence in trg_sent]
        sample = self.task.build_dataset_for_clm(src_tokens, len(src_tokens), trg_tokens, len(trg_tokens))
        sample = sample.collater(sample)
        sample = utils.apply_to_sample(lambda t: t.to(self.device), sample)
        loss, _, _ = self.task.valid_step(sample, self.models[0], criterion)
        return loss
