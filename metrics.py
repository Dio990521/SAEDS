...
if __name__ == "__main__":
    ...
    embeddings, word2id, word_list = read_total_embeddings('sgns.weibo.char', max_vocab_size)
    embeddings = get_norm_embeddings(embeddings)

    print("read response data")
    generate_response = read_response_file('test_response.txt', word2id)
    true_response = read_response_file('reference.txt', word2id)
    
    print("compute metrics")
    compute_distinct_metric('test_response.txt')
